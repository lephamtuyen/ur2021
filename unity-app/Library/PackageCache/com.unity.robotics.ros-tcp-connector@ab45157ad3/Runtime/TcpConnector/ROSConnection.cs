...
public class ROSConnection : MonoBehaviour
{
    ...
    public async void SendServiceMessage<RESPONSE>(int robotid, string rosServiceName, Message serviceRequest, Action<RESPONSE ,int> callback) where RESPONSE : Message,  new()
    {
        int robot_id = robotid;
        ...
        while (!networkStream.DataAvailable && attempts <= this.awaitDataMaxRetries)
        {
            ...
            {
                Debug.Log("No data available on network stream after " + awaitDataMaxRetries + " attempts.");
                goto finish;
            }
            attempts++;
        ...
        finish:
        callback(serviceResponse,robot_id);
    ...
    private void Start()
    {
        ...
        SendServiceMessage<UnityHandshakeResponse>(0,HANDSHAKE_TOPIC_NAME, new UnityHandshakeRequest(overrideUnityIP, (ushort)unityPort), RosUnityHandshakeCallback);
    }
    ...
    void RosUnityHandshakeCallback(UnityHandshakeResponse response,int meaningless=0)
    {
