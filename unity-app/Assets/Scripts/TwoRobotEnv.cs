...
public class TwoRobotEnv : Agent
{
    ...
    void FixedUpdate() 
    {
        ...
        if (initilized && conveyorStarted && (!planner.moving[0] || !planner.moving[1]) && planner.messageNum < 1 && planner.reset_robot)
        {
            ...
            {
                // for(int robotid = 0; robotid < planner.niryoOne.Length; robotid++)
                // {
                //     if (!planner.moving[robotid] && planner.messageNum < 1)
                //     {
                //         if (CanPlan(robotid))
                //         {
